...
Since Dan is also lead developer of the Qrack quantum computer simulation framework, he has wrapped that framework as a plugin for OpenRelativity, here, so that "conformal" quantum mechanics can immediately be employed in our relativity simulations. **"Conformal" quantum computer programs** can be written with instructions "pinned" to the relativistic notions of distance and time of OpenRelativity, by overloading the `RealTimeQasmProgram` abstract class. (When we say "conformal," we mean basically that special relativity or "Minkowski" simulation, as well as quantum mechanics or quantum computation simulation, "conforms" to the background curvature of a `ConformalMap` as if these mechanics happened in a locally approximately "flat" space that is "bent" over larger scales to conform to the boundary of the underlying space-time, in accordance with "Einstein equivalence principle." Also, the "`RealTimeQasmProgram`" API is not actually proper "QASM," but it is a similar quantum assembly or intermediate representation.)

## Note about accelerator APIs for developers

If you are a developer, note that [Qrack](http://github.com/unitaryfoundation/qrack) offers many build customization options for a bespoke build that likely fits your specific needs for Linux, Android, Mac, Windows, or iOS. Qrack hardware acceleration is available as either OpenCL (cross-vendor compatible) or CUDA (locked into NVIDIA hardware), and these two APIs "translate" nearly one-for-one, line-by-line for equivalent implementations, besides some JIT "boilerplate" code for OpenCL. If desired, both OpenCL and CUDA builds are fully supported for all of these operating systems. (Open an issue ticket as appropriate on this repository or Qrack's, if you need help with custom builds from source.) But, as Mac and Windows have both become more-or-less antagonistic or at least indifferent toward the OpenCL API, and since **choice of CUDA architecture variant support needs to be specified at _compile-time,_** you likely need to determine your accelerator market-share support matrix for yourself and build Qrack accordingly, on Mac or Windows. The Qrack team is a friendly bunch, and we're here to help with questions, always.
...
```json
{
  "dependencies": {
    ...
    "net.qrack.tachyoid": "https://github.com/vm6502q/OpenRelativity.git?path=/Packages/net.qrack.tachyoid",
